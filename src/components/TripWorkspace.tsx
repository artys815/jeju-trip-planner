...
import type { Day, Itinerary, Trip } from '../types'
import { isItinerary } from '../types'
...
import { LiveStickyBar } from './LiveStickyBar'
import { LiveTestBanner } from './LiveTestBanner'
import { NoticeSection } from './NoticeSection'
import { ResetModal } from './ResetModal'
import { RestaurantSection } from './RestaurantSection'
import { TodayShortcut } from './TodayShortcut'
...
interface TripWorkspaceProps {
  ...
  onBackHome: () => void
  onResetActive: () => void
  onRecoverLegacy: () => { ok: boolean; reason?: string }
}
...
export function TripWorkspace({
  ...
  const isProtectedJeju = Boolean(trip.protected)
  ...
  const handleExport = () => {
    ...
    a.href = url
    const safeName = (trip.name || 'trip').replace(/[\\/:*?"<>|]+/g, '_').slice(0, 40)
    a.download = `${safeName}-itinerary.json`
    a.click()
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      if (!isItinerary(parsed)) {
        setError(
          '불러온 JSON 형식이 올바르지 않습니다. 일정 데이터 구조를 확인해 주세요.',
        )
        return
      }
      const replace = window.confirm(
        '현재 여행의 일정만 JSON으로 교체합니다. 다른 여행은 영향받지 않습니다. 계속할까요?',
      )
      if (!replace) return
      setItinerary(parsed)
      setDraft(null)
      ...
      setError(
        'JSON 파일을 읽지 못했습니다. 파일이 손상되었거나 형식이 잘못되었습니다.',
      )
    }
  }
  ...
  const handleRecoverLegacy = () => {
    if (!isProtectedJeju) return
    const ok = window.confirm(
      '기존 제주 일정 백업으로 현재 여행을 복구합니다.\n현재 여행의 일정 내용이 교체됩니다. 계속할까요?',
    )
    if (!ok) return
    const result = onRecoverLegacy()
    if (!result.ok) {
      setError('복구할 기존 제주 백업을 찾지 못했습니다.')
      return
    }
    setDraft(null)
    setResetMessage('기존 제주 데이터로 복구했습니다.')
  }

  const handleBackHome = () => {
    if (isEditing && hasUnsavedChanges && !confirmDiscardIfNeeded()) return
    setDraft(null)
    onBackHome()
  }
  ...
  const showTravelDayLive = Boolean(
    todayDay &&
      liveStatus &&
      !isEditing &&
      (realTodayIndex >= 0 || testMode.enabled),
  )
  ...
  return (
    <div
      className={[
        ...
        .join(' ')}
    >
      <div className="trip-nav no-print">
        <button type="button" className="btn btn--ghost trip-nav__back" onClick={handleBackHome}>
          ← 내 여행
        </button>
        {isProtectedJeju ? (
          <button
            type="button"
            className="btn btn--ghost trip-nav__recover"
      ...
      <main className="main">
        ...
        <Toolbar
          ...
          resetLabel={
            isProtectedJeju ? '제주 기본 일정 복원' : '빈 일정으로 초기화'
          }
      ...
      <ResetModal
        step={resetStep}
        mode={isProtectedJeju ? 'jeju-default' : 'empty'}
